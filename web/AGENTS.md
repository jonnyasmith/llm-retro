...
- **`pnpm test:visual`** — visual regression: screenshots every story in the pinned Playwright Linux image and diffs against committed baselines. `pnpm test:visual:update` regenerates them. Baselines are Linux-only by design (see [`docs/adr/0007`](docs/adr/0007-self-hosted-visual-regression.md)); a "Visual tests" Storybook panel reviews diffs.
