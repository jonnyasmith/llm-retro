...
- **`pnpm test:visual`** — visual regression preview: screenshots every story in the pinned Playwright image (`--platform=linux/amd64`) and diffs against committed baselines. A "Visual tests" Storybook panel reviews diffs. **Baselines are generated by CI, not locally** — Apple-Silicon emulation can't reproduce native x86 text rasterisation, so run `gh workflow run visual.yml -f update=true --ref <branch>` to regenerate and commit them. See [`docs/adr/0007`](docs/adr/0007-self-hosted-visual-regression.md).
