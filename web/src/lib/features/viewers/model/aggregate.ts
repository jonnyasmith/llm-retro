...
import type { InferenceType, ResolvedTheme, Session, Theme } from './session-types';
...
export function filtered(all: readonly Session[], state: FilterState): Session[] {
	const cut = new Date(state.now.getTime() - state.days * 86400000);
	return all.filter(
		(s) =>
...
export function iScope(f: readonly Session[], sourceThemes: readonly Theme[]): InsightScope {
	...
	const themes = sourceThemes
		.map((t) => ({ ...t, sessions: f.filter(t.match) }))
