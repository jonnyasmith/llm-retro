...
- **Adapter (per Harness) owns** — enumerating its source-file groups (its directory topology, and Codex's relocation into `archived_sessions`), deriving the `stableSessionId`, and parsing a text slice into a normalised Session (genuine-prompt detection, sub-agent fold-up + main/sub split, the stable `interaction_key`). Of token extraction it owns only the genuinely Harness-shaped part — the wire-key mapping, which record field means which bucket. Accumulating those values under the null-not-zero rule, and resolving which Model served an Interaction, are one shared implementation each: they are domain rules, identical across Harnesses, and four private copies is how they drifted.
...
**Naming.** _Adapter_ means the strategy object above and nothing else. The per-Harness module that knows one log format's record grammar — how to read a Session's metadata, find a prompt boundary, extract usage from a record — is a **log reader**, which the adapter delegates to. Where two Harnesses share a wire format, one log reader owns the grammar and the other _delegates_ to it rather than restating it: omp's records are pi-shaped, so omp's log reader drives pi's grammar, wrapping it to thread its own sub-agent logs through and to relax the one point they disagree on — whether the `session` record must come first. Delegation, not shared ownership, because the borrower always has a residue of its own.

The **Interaction accumulator** is the shared module that turns a log reader's records into normalised Interactions: open on a genuine prompt (ADR-0001), gather what answered it, then settle the Interaction's serving Model and Token usage. It publishes the accumulation itself, the timestamp-envelope fold, the prompt-boundary finder, and the sub-agent tree walk. It stays Harness-agnostic — no `if (harness === …)` branch — because each log reader configures it with a **dialect**: the genuine-prompt predicate, the main-assistant predicate, the Interaction opener, and the wire-key map, declared once at module scope so ingesting a Session allocates no configuration. pi, omp, and Claude share it. Codex does not: its absorbed `turn_context` markers and cumulative Token usage (ADR-0010) are a different algorithm, and forcing it in would be over-parameterisation.
...
The concrete adapter decisions the seam produced, each grilled against real logs on disk. They live here, not in the code, because they are the _why_ behind otherwise surprising parsing choices.

**pi** — sub-agent tokens are never logged (a `subagent` toolResult is plain text: no `usage`, no nested assistant records, no agent marker), so `subTokens` is null (the null-object case) and the Interaction carries a **`spawnedSubagents`** flag marking its token total a floor, not a full accounting. Model is read directly off each assistant record's `message.model`; the separate `model_change` record is a redundant UI event and is ignored. A multi-model Interaction resolves to one Model by the serving-model rule — canonicalise every candidate spelling _first_, then most output tokens wins. The ordering is the whole rule: tallying on the raw string makes two spellings of one Model split its vote and discards the loser's tokens. Genuine prompt = `message.role === "user"`; `interaction_key` = the message `id`.
...
- Concerns that look cross-cutting but are Harness-specific stay inside the adapter: `token_count` cumulative-vs-delta correlation (Codex), where sub-agent tokens live (Claude sibling files, omp nested files, pi flat). The pipeline never learns of them. The sub-agent _topologies_ stay — Claude resolves a child by a Session-wide agent id, omp by a name scoped to its parent's directory — but the depth-first _traversal_ over them does not: that is one walk in the Interaction accumulator, configured by each Harness's topology.
