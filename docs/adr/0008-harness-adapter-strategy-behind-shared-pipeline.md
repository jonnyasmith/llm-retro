...
**Naming.** *Adapter* means the strategy object above and nothing else. The per-Harness module that knows one log format's record grammar — how to read a Session's metadata, find a prompt boundary, extract usage from a record — is a **log reader**, which the adapter delegates to. Where two Harnesses share a wire format, one log reader owns the grammar and the other *delegates* to it rather than restating it: omp's records are pi-shaped, so omp's log reader drives pi's grammar, wrapping it to thread its own sub-agent logs through and to relax the one point they disagree on — whether the `session` record must come first. Delegation, not shared ownership, because the borrower always has a residue of its own.
...
**pi** — sub-agent tokens are never logged (a `subagent` toolResult is plain text: no `usage`, no nested assistant records, no agent marker), so `subTokens` is null (the null-object case) and the Interaction carries a **`spawnedSubagents`** flag marking its token total a floor, not a full accounting. Model is read directly off each assistant record's `message.model`; the separate `model_change` record is a redundant UI event and is ignored. A multi-model Interaction resolves to one Model by the serving-model rule — canonicalise every candidate spelling *first*, then most output tokens wins. The ordering is the whole rule: tallying on the raw string makes two spellings of one Model split its vote and discards the loser's tokens. Genuine prompt = `message.role === "user"`; `interaction_key` = the message `id`.
...
- Concerns that look cross-cutting but are Harness-specific stay inside the adapter: `token_count` cumulative-vs-delta correlation (Codex), where sub-agent tokens live (Claude sibling files, omp nested files, pi flat). The pipeline never learns of them. The sub-agent *topologies* stay — Claude resolves a child by a Session-wide agent id, omp by a name scoped to its parent's directory — but the depth-first *traversal* over them does not: that is one walk in the Interaction accumulator, configured by each Harness's topology.
