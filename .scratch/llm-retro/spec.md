...
- Genuine-user-prompt detection: **resolved for Claude** (see reference above; an Interaction also requires ≥1 assistant response — ADR-0001). Still to solve per remaining harness (pi, Codex, omp).
- Interaction idempotency key — **resolved**: `UNIQUE(sessionId, openingUserRecordId)` (S2 schema), i.e. session + opening genuine-user-record id.
- Codex `token_count` cumulative vs delta; mixed-model Interaction handling (Claude takes first assistant model, else max-output "serving" model — carry the serving-model rule to Codex).
